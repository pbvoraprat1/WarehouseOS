...
import { UpdateWarehouse, getAllWarehouse } from "../lib/api";
...
export default function EditWarehouseModal({ isOpen, onClose, warehouse }) {
    ...
    const updateWarehouseMutation = useMutation({
        mutationFn: (updatedWarehouse) => UpdateWarehouse({ id: warehouse.id, payload: updatedWarehouse }),
        onSuccess: () => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-card w-full max-w-md rounded-xl shadow-2xl border border-border overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between px-6 py-4 border-b border-border/50">
                    <h2 className="text-lg font-semibold text-foreground">Edit Warehouse</h2>
                    <button
                        onClick={handleClose}
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border/50 bg-muted/20">
                        ...
                        <button
                            ...
                            disabled={updateWarehouseMutation.isPending}
                        >
                            {updateWarehouseMutation.isPending && <Loader2 className="w-4 h-4 animate-spin" />}
                            Save Changes
                        </button>
                    </div>
                </form>
