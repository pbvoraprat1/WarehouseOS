...
export default function Dashboard() {
  ...
  const dropdownDisplayLabel = activeWarehouse
    ? activeWarehouse.name
    : dbData?.warehouse_name || "เลือกคลังสินค้า";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Monitor inventory levels and recent activity.
          </p>
        </div>
        ...
        {[
          {
            ...
            value: dbData.summary.total_low_stock,
            icon: TrendingDown,
            color: "text-warning",
          },
          {
            label: "Out of Stock (Critical)",
            value: dbData.summary.out_of_stock,
            icon: XCircle,
            color: "text-destructive",
          },
          {
            label: "Near Out of Stock",
            value: dbData.summary.near_out_of_stock,
            icon: AlertTriangle,
            color: "text-warning",
          },
        ].map((card) => (
